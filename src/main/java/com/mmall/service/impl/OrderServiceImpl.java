...
public class OrderServiceImpl implements IOrderService {
    ...
    public ServiceResponse<OrderVo>getOrderDetail(Integer userId,Long orderNo){
        ...
        return ServiceResponse.createByErrorMsg("没有找到订单");
    }
    ...
    public ServiceResponse<OrderProductVo> getOrderCartProduct(Integer userId){
    ...
    public ServiceResponse<String> cancel(Integer userId,Long orderNo){
        Order order = orderMapper.selectByUserIdOrderNo(userId,orderNo);
        if(order == null){
            return ServiceResponse.createByErrorMsg("该订单不存在");
        }
        // 二期会通过支付宝的一个回调来解决
        if(order.getStatus() != Const.OrderStatusEnum.NO_PAY.getCode()){
            return ServiceResponse.createByErrorMsg("已付款，无法取消订单");
        }
    ...
    public ServiceResponse createOrder(Integer userId,Integer shippingId){
        ...
        ServiceResponse serviceResponse = this.getCartOrderItem(userId,cartList);
        if(!serviceResponse.isSuccess()){
            return ServiceResponse.createByErrorMsg("生成订单错误");
        }
        List<OrderItem>orderItemList = (List<OrderItem>)serviceResponse.getData();
        if(CollectionUtils.isEmpty(orderItemList)){
            return ServiceResponse.createByErrorMsg("购物车为空");
        }
        BigDecimal payment = this.getOrderTotalPrice(orderItemList);

        // 生成订单
        Order order = this.assembleOrder(userId,shippingId,payment);
        if(order == null) return ServiceResponse.createByErrorMsg("生成订单失败");
    ...
    private List<OrderVo> assembleOrderVoList(List<Order>orderList,Integer userId){
        ...
        for(Order order : orderList){
            ...
            if(userId == null){
                ...
                orderItemList = orderItemMapper.selectOrderItemListByOrderNo(order.getOrderNo());
            }else{
                orderItemList = orderItemMapper.selectOrderItemListByUserIdOrderNo(userId,order.getOrderNo());
            }
            OrderVo orderVo = assembleOrderVo(order,orderItemList);
    ...
    private ServiceResponse<List<OrderItem>>getCartOrderItem(Integer userId,List<Cart>cartList){
        ...
        // 校验购物车的数据，包括产品是否上线和库存
        for(Cart cart : cartList){
            OrderItem orderItem = new OrderItem();
            Product product = productMapper.selectByPrimaryKey(cart.getProductId());
            // 上线情况
            if(Const.ProductSaleEnum.ON_SALE.equals(product.getStatus())){
                return ServiceResponse.createByErrorMsg("产品"+product.getName()+"不在线售卖");
            }
            //检查库存
            if(product.getStock() < cart.getQuantity()){
                return ServiceResponse.createByErrorMsg("产品"+product.getName()+"库存不够");
            }
    ...
    public ServiceResponse pay(Long orderNo,Integer userId,String path){
        ...
        // 首先要验证根据userId是否是对应这个订单号
        Order order = orderMapper.selectByUserIdOrderNo(userId,orderNo);
        if(order == null){
            ServiceResponse.createByErrorMsg("用户没有该订单");
        }
        ...
        String subject = new StringBuilder().append("happymmall商城扫码").append(order.getOrderNo()).toString();
        ...
        switch (result.getTradeStatus()) {
            case SUCCESS:
                ...
                String qrName = String.format("qr-%s.png",
                        response.getOutTradeNo());
            ...
            case FAILED:
                ...
                return ServiceResponse.createByErrorMsg("支付宝预下单失败!!!");

            case UNKNOWN:
                logger.error("系统异常，预下单状态未知!!!");
                return ServiceResponse.createByErrorMsg("系统异常，预下单状态未知!!!");

            default:
                logger.error("不支持的交易状态，交易返回异常!!!");
                return ServiceResponse.createByErrorMsg("不支持的交易状态，交易返回异常!!!");
        }
    }
...
//    校验通知中的 seller_id（或者seller_email) 是否为 out_trade_no 这笔单据的对应的操作方（有的时候，一个商户可能有多个 seller_id/seller_email）。
    public ServiceResponse aliCallBack(Map<String,String>params){
        Long orderNo = Long.parseLong(params.get("out_trade_no")); //商城的订单号
        String tradeNo = params.get("trade_no");// 支付宝的订单号
        String tradeStatus = params.get("trade_status");

        Order order = orderMapper.selectByOrderNo(orderNo);
        if(order == null){
            return ServiceResponse.createByErrorMsg("非本商城订单，回调失败");
        }
    ...
    public ServiceResponse queryOrderPayStatus(Integer userId,Long orderNo){
        Order order = orderMapper.selectByUserIdOrderNo(userId,orderNo);
        if(order == null){
            return ServiceResponse.createByErrorMsg("用户没有该订单");
        }
        if(order.getStatus() >= Const.OrderStatusEnum.PAID.getCode()){
            return ServiceResponse.createBySuccess();
        }
    ...
    public ServiceResponse<OrderVo> manageDetail(Long orderNo){
        ...
        return ServiceResponse.createByErrorMsg("没有该订单");
    }
    ...
    public ServiceResponse<String>manageSendGoods(Long orderNo){
        ...
        return ServiceResponse.createByErrorMsg("订单不存在");
    }
}
